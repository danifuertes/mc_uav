...
class PointerNetwork(nn.Module):
    ...
    def _inner(self, inputs, eval_tours=None):
        ...
        h0 = c0 = Variable(
            torch.zeros(1, batch_size, self.hidden_dim, out=node_embedding.data.new()), requires_grad=False
        )  # Variable is deprecated, use Tensor
        context, (enc_h0, enc_c0) = self.lstm_enc(node_embedding, (h0, c0))
        ...
        while not state.all_finished():
            ...
            # LSTM (decoder)
            h, c = self.lstm_dec(x, hidden_dec)
            hidden_dec = (h, c)
            ...
            if self.is_op:
                ...
                context, hidden_enc = self.lstm_enc(node_embedding, hidden_enc)
